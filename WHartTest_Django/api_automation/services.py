    ...
    for assertion in assertions or [{"type": "status_code", "operator": "lt", "expected": 500}]:
        ...
        if typ == "status_code":
            ...
            header_name = str(
                ...
                or assertion.get("header")
                or (expected if isinstance(expected, str) else "")
            ).lower()
            actual = next(
            ...
            actual = _get_by_path(parsed_body, assertion.get("path") or assertion.get("target") or "")
            passed = _compare(actual, operator, expected)
        elif typ == "json_path_length":
            if parsed_body is None:
                parsed_body = _safe_response_json(response)
            value = _get_by_path(parsed_body, assertion.get("path") or assertion.get("target") or "")
            actual = len(value) if isinstance(value, (list, dict, str, tuple, set)) else None
            passed = _compare(actual, operator, expected)
